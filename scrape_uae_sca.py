#!/usr/bin/env python3
"""
Sitemap-based scraper for www.uaecma.gov.ae (UAE Securities and Commodities Authority).
...
Usage:
    python3 scrape_uae_sca.py              # incremental run
    python3 scrape_uae_sca.py --force      # re-fetch everything
    python3 scrape_uae_sca.py --slug X     # fetch only slug X
"""
...
# --- Constants ---
DOMAIN = "www.uaecma.gov.ae"
SITEMAP_URL = "https://www.uaecma.gov.ae/sitemap.xml"
BASE_DIR = Path(__file__).resolve().parent / "uae_sca"
...
CONTENT_PATH_PATTERNS = [
    ...
    # UAE SCA-specific patterns
    "/en/Pages/", "/ar/Pages/",
    "/en/media-center/", "/ar/media-center/",
    "/en/about-sca/", "/ar/about-sca/",
    "/en/regulations/", "/ar/regulations/",
    "/en/decisions/", "/ar/decisions/",
    "/en/circulars/", "/ar/circulars/",
    "/en/investor-awareness/", "/ar/investor-awareness/",
    "/en/legislations/", "/ar/legislations/",
    "/en/services/", "/ar/services/",
    "/en/market-data/", "/ar/market-data/",
]
...
def detect_content_type(url_path):
    ...
    if not content_type:
        segments = [s for s in path.split("/") if s and not re.match(r"^\d{4}$", s) and s not in ("en", "ar", "pages")]
        if len(segments) >= 2:
...
def find_content_container(soup):
    ...
    # UAE SCA-specific selectors (likely Sitecore/Kentico/.NET CMS)
    selectors = [
        ...
        "div.main-content",
        "div.page-content",
        # .NET / SharePoint patterns common in UAE gov sites
        "div#MainContent",
...
def replace_internal_links():
    ...
    pattern = re.compile(
        r"\[([^\]]+)\]\(https?://(?:www\.)?" + re.escape(DOMAIN.replace("www.", "")) + r"(/[^)\s]*)\)"
    )
