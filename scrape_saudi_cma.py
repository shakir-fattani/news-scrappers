#!/usr/bin/env python3
"""
Sitemap-based scraper for cma.gov.sa (Saudi Capital Market Authority).
...
Usage:
    python3 scrape_saudi_cma.py              # incremental run
    python3 scrape_saudi_cma.py --force      # re-fetch everything
    python3 scrape_saudi_cma.py --slug X     # fetch only slug X
"""
...
# --- Constants ---
DOMAIN = "cma.gov.sa"
SITEMAP_URL = "https://cma.gov.sa/sitemap.xml"
BASE_DIR = Path(__file__).resolve().parent / "saudi_cma"
...
CONTENT_PATH_PATTERNS = [
    ...
    # CMA-specific patterns
    "/en/market/", "/en/aboutcma/", "/en/awareness/", "/en/mediacenter/",
    "/en/rulebook/", "/en/legislations/", "/en/circular/",
    "/ar/market/", "/ar/aboutcma/", "/ar/awareness/", "/ar/mediacenter/",
    "/ar/rulebook/", "/ar/legislations/", "/ar/circular/",
    "/en/Pages/", "/ar/Pages/",
]
...
def should_skip_url(url):
    """Check if URL should be skipped (pagination, tags, etc.)."""
    parsed = urlparse(url)
...
def is_listing_page(url, soup, text_content):
    ...
    # URL ends with just a content-type pattern
...
def detect_content_type(url_path):
    ...
    if not content_type:
        segments = [s for s in path.split("/") if s and not re.match(r"^\d{4}$", s) and s not in ("en", "ar")]
        if len(segments) >= 2:
...
def generate_slug(url_path):
    ...
    # Filter out date segments and language prefixes
    meaningful = []
    for seg in segments:
...
def _normalize_date(date_str):
    ...
    # RFC 2822 or other common formats
...
def _extract_date_from_url(soup):
    """Try to extract date from canonical URL or og:url."""
    url = None
...
def html_to_markdown(element, slug, depth=0):
    ...
    # Skip noise elements by class/id
    ...
    if tag == "a":
        ...
        # If wrapping an image, just recurse
        if element.find("img"):
            return children_md
        text = children_md.strip()
        if text and href:
...
def find_content_container(soup):
    """Find the main content container using platform-specific then generic selectors."""
    # CMA-specific selectors
    selectors = [
        ...
        "div.main-content",
        "div.page-content",
        # SharePoint/CMS patterns common in gov sites
...
def clean_markdown(md):
    """Clean up markdown: collapse excessive newlines, strip trailing spaces."""
    md = re.sub(r"\n{3,}", "\n\n", md)
    md = re.sub(r" +\n", "\n", md)
    md = md.strip()
    return md
...
def process_page(entry, state, force=False):
    """Fetch and process a single page. Returns (slug, success)."""
    url = entry["loc"]
...
def replace_internal_links():
    ...
    pattern = re.compile(
        r"\[([^\]]+)\]\(https?://(?:www\.)?" + re.escape(DOMAIN) + r"(/[^)\s]*)\)"
    )
